...
async def run_translation_pipeline_async(api_key:str, base_url:str, model1:str,
    ...
    for start in range(0, total, batch_size):
        ...
        sys_prompt = """
        你是熟悉 QGIS 與 GIS 領域的台灣在地化譯者。

        【任務】
        - 將每一個項目中欄位 text 的英文 UI 文字，翻譯成自然的繁體中文（台灣用語）。

        【重要規則】
        1. 只翻譯 text 欄位的內容。
          - 可以參考 context 與 glossary 來理解意思，
          - 但絕對不要把 context 的文字（例如「介面: Animation3DWidget」「註釋: ...」）直接抄進輸出。

        2. 必須完整保留所有 placeholder 與標記：
          - 例如：⟦M0⟧、%L1、%1、%n、%s、{0}、{1}、HTML / XML 標籤（如 <b>、<a>）。
          - 不可以新增、刪除或改變順序，也不要改變大小寫。

        3. 儘量保留原文中的半形特殊符號：
          - 例如: + - / : () [] <> = % 等，避免改成全形，也不要隨意多加或少掉符號。

        4. 若字串看起來是常數、enum 名稱或函式名稱（例如 InQuad, OutCubic, LayerMode_SVG, QgsVectorLayer）：
          - 優先保留英文原文不翻；
          - 若真的需要說明，可以在不破壞原字的情況下，少量補上說明（例如「InQuad（二次方緩入）」），
          - 若不確定，寧可完全保留英文。

        5. 若沒有合適或確定的中文翻譯，寧可保留英文原文，不要亂造詞。

        【輸出方式】
        - 請呼叫工具 set_results，
        - 只在 results 陣列中，依照輸入順序填入對應的翻譯字串，
        - 不要額外加註解或說明。
        """
                    ...
                    "messages": [
                        {
                            "role":"system",
                            "content": "content": """
                            你是嚴格的台灣在地化校對員，負責在兩個候選譯文 A 與 B 之間擇優，並在必要時進行小幅修正。

                            【輸入格式】
                            - 使用者會給你一個 JSON 陣列，每個元素都包含：
                              - src: 原始英文文字（UI 字串）
                              - ctx: context 說明（介面名稱、註解等）
                              - A: 第一版譯文
                              - B: 第二版譯文

                            【你的工作】
                            對每一個元素，請依照下列優先順序處理：
                            1. 檢查 A 和 B 是否有漏翻、亂翻、語意錯誤或不自然的地方。
                            2. 檢查格式與結構是否正確：
                              - 所有 placeholder（例如 ⟦M0⟧、%L1、%1、%n、{0} 等）都必須完整保留且順序相同。
                              - HTML / XML 標籤與屬性必須保持正確配對與結構，不可遺失或移位。
                              - 半形符號（例如 + - / : () [] <> = %）不要隨意改成全形，也不要亂增刪。

                            3. 從 A 與 B 中先選出較好的一個作為基礎：
                              - 如果其中一個譯文明顯更忠實、自然且格式正確，就以那一個為主。
                              - 若兩個都還可以，擇其較清楚、較自然的一個。

                            4. 在選定的那個譯文上做必要的小幅修正：
                              - 修正明顯錯誤或漏翻。
                              - 統一用語，使其符合台灣常用說法。
                              - 但不要大幅改寫成完全不同的句子，也不要添加新的意思。

                            5. 絕對不要把 ctx（context）裡的文字直接抄進輸出。
                              - ctx 只能用來理解情境，不是翻譯內容的一部分。

                            6. 如果 A 和 B 兩個譯文都非常糟，無法信任：
                              - 就直接輸出英文原文 src（不做翻譯），以避免產生錯誤的中文。

                            【輸出方式】
                            - 對於輸入陣列中的每個元素，依照順序給出最後選擇／修正後的單一句子結果。
                            - 請呼叫工具 set_results，並只在 results 陣列中依序放入你最終決定的譯文。
                            - 不要輸出任何解釋或說明文字。
                            """
                        },
